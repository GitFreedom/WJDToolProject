...
+ (WJDTimer *)timerWithTimeInterval:(NSTimeInterval)interval
    ...
    return [[self alloc] initWithFireTime:interval timeInterval:interval target:target selector:selector repeats:repeats];
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"YYTimer init error" reason:@"Use the designated initializer to init." userInfo:nil];
    return [self initWithFireTime:0 timeInterval:0 target:self selector:@selector(invalidate) repeats:NO];
}
...
- (instancetype)initWithFireTime:(NSTimeInterval)fireTime
    ...
    dispatch_source_set_timer(_source, dispatch_time(DISPATCH_TIME_NOW, (fireTime * NSEC_PER_SEC)), (interval * NSEC_PER_SEC), 0);
